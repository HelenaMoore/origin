...
Rhombus::Rhombus(int sides_count, int side_a, int side_b, int side_c, int side_d, int angle_A, int angle_B, int angle_C, int angle_D) : Parallelogram(sides_count, side_a, side_b, side_c, side_d, angle_A, angle_B, angle_C, angle_D)
    {
        name = "Ромб";
        ...
        if (angle_C != angle_A || angle_D != angle_B)
        {
            throw BadFormException(name + " (стороны " + std::to_string(side_a) + ", " + std::to_string(side_b) + ", "
                + std::to_string(side_c) + ", " + std::to_string(side_d) + "; углы " + std::to_string(angle_A) + ", "
                + std::to_string(angle_B) + ", " + std::to_string(angle_C) + ", " + std::to_string(angle_D)
                + ") не был создан. Причина: Углы попарно не равны");
        }
    }